...
contract DDW is ERC721A, EIP712, ERC2981, Ownable, Pausable, ReentrancyGuard {
    ...
    address public constant openSeaProxyRegistryAddress = 0xF57B2c51dED3A29e6891aba85459d600256Cf317; 
    // Rinkeby Network Opensea Contract Address, put mainnet for Deployment
    ...
    function becomeAChad() external payable callerIsUser nonReentrant {
        ...
        require(totalSupply() + MAX_MINT_PER_WALLET < MAX_SUPPLY_plus_1, "Become a Chad on Secondary Market");
        require(msg.value > publicMintPrice, "You dont have what it takes to become a Chad");
    ...
    function becomeAPartnerChad(bytes memory signature) external payable callerIsUser nonReentrant {
        require(checkIfPrivileged(msg.sender, signature) == privilegedMintSignerAddress,
                "Its not easy to become a Mega Partner Chad"
                );
        require(
        !hasMinted[msg.sender],
        "You are already a Chad"
    ...
    function becomeAR2EChad(uint256 tokenId, uint256 mintPriceInWei, bytes memory signature) external payable callerIsUser nonReentrant {
        require(checkIfReferral(msg.sender, mintPriceInWei, tokenId, signature) == referralMintSignerAddress,
                "Its not easy to become a R2E Chad"
                );
        require(
        isReferralMintLive,
        "You will become a Chad soon"
        );
        require(mintPriceInWei > minReferralMintPrice, "Your Mega Partner Chad is Kidding with you");
        require(msg.value > mintPriceInWei, "You dont have what it takes to become a Chad");   
        require(this.isTokenPrivileged(tokenId), 
                "It requires a Mega Partner Chad for you to become an R2E Chad"
                );
        require(
        !hasMinted[msg.sender],
        "You are already a Chad"
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI_).length != 0 ? string(abi.encodePacked(baseURI_, "PrivilegedChadCard.json")) : '';
        return bytes(baseURI_).length != 0 ? string(abi.encodePacked(baseURI_, "ChadCard.json")) : '';
    }
